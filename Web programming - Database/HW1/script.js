...
function fetchArtisti(){
    fetch("artisti.php").then(fetchResponse).then(fetchArtistiJson);
}
...
function fetchArtistiJson(json){
    ...
    for(let artisti of json)
    {
        ...
        const biglietti = document.createElement("input");              //Si crea un elemento che serve per l'acquisto del biglietto
        biglietti.setAttribute("class", "Biglietti");
        biglietti.setAttribute("type", "submit");
        biglietti.setAttribute("value", "Biglietti");
        biglietti.setAttribute("name", "submit");
        biglietti.textContent = "Acquista";
